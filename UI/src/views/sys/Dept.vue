<template no-gutters>
  ...
  <v-container class="main-page-wrapper">
    ...
    <v-row>
      ...
      <v-col cols="6">
        <v-card class="flex-grow-1 bg-white" variant="outlined">
          <v-card-title> {{ selectedDept.deptPath }}  </v-card-title>
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-select
                v-model="selectedDept.upCode"
                :items="filteredDeptCode"
                item-title="deptName"
                item-value="deptno"
                label="상위부서"
                variant="outlined"
          ...
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-text-field v-model="selectedDept.deptno" :readonly="viewMode !== 'new'"
                ref="deptno" maxlength="4" hide-details variant="outlined" label="부서코드" placeholder="부서코드를 입력하세요" >
              </v-text-field>
            </v-col>
          </v-row>
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-text-field v-model="selectedDept.deptName" :readonly="isReadonly && viewMode == 'view'"
                ref="deptName" maxlength="50" hide-details variant="outlined" label="부서명" placeholder="부서명을 입력하세요" >
              </v-text-field>
            </v-col>
          </v-row>
          ...
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-text-field v-model="selectedDept.hostname" :readonly="isReadonly && viewMode == 'view'"
                maxlength="8" hide-details variant="outlined" label="책임자" placeholder="책임자를 입력하세요" >
              </v-text-field>
            </v-col>
          </v-row>
          <!-- :readonly="isReadonly && viewMode === 'view'" -->
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-select
                v-model="selectedDept.deptGubun"
                :items="orgType"
                item-title="codeName"
          ...
          <v-row no-gutters justify="center" align="center" class="ma-2">
            <v-col>
              <v-text-field v-model="selectedDept.deptOrder" :readonly="isReadonly && viewMode == 'view'"
                type="number" hide-details variant="outlined" label="정렬순서" placeholder="정렬순서를 입력하세요" >
              </v-text-field>
            </v-col>
          </v-row>
          <v-row no-gutters justify="center" align="center" class="ma-2">
...
export default {
  ...
  methods: {
    ...
    getDept() {
      ...
      const inputData = {    
        comCodes: [
          { groupCode: "117", codeName: "orgType", useYn: 'Y' },  // 조직구붖
          { groupCode: "155", codeName: "useType", useYn: 'Y' },  // 사용여부구분
        ]    
      };
