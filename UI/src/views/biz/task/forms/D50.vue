    ...
    <v-form ref="formSuccess">
      <v-sheet rounded="lg" class="pa-4 mb-3 form-card">
        ...
        <div class="field-item mb-4">
          <span class="field-label required">요약정보</span>
          <v-textarea
            v-model="model.bizInfo.successSummary"
            variant="outlined"
            density="compact"
            rows="3"
            no-resize
        ...
        <!-- 낙찰/계약결과 -->
        <div class="field-item mb-4">
          <span class="field-label required">낙찰/계약결과</span>
          <v-textarea
            v-model="model.bizInfo.successResult"
            variant="outlined"
            density="compact"
            rows="3"
            no-resize
        ...
        <v-row v-if="!isCompleted" no-gutters class="d-flex justify-end align-center flex-grow-0">
          <div class="d-flex ga-1">
            <v-btn variant="flat" text="저장" @click="handleSaveSuccess"/>
            <v-btn variant="flat" text="결재"/>
          </div>
        </v-row>
...
const handleSaveSuccess = async () => {
  const { valid } = await formSuccess.value.validate();
  if (!valid) return;
  saveSection('success');
};
...
const ALL_FIELDS = [...MANAGER_FIELDS, ...SUCCESS_FIELDS];

const saveSection = (section, overrides = {}) => {
  const fieldMap = { manager: MANAGER_FIELDS, success: SUCCESS_FIELDS };
