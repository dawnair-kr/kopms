...
import { createElement, detach } from '@syncfusion/ej2-base/src/dom.js';
import { extend, getUniqueID, getValue, isNullOrUndefined } from '@syncfusion/ej2-base/src/util.js';
import { getTemplateEngine, setTemplateEngine } from '@syncfusion/ej2-base/src/template-engine.js';
...
let ComponentBase = vueDefineComponent({
    ...
    methods: {
        ...
        assignValueToWrapper(option, silent) {
            //console.log("sssssssss", option, silent)
        ...
        if (context) {
            ...
            if (!isExecute && (typeof templateElement === "string" || (templateElement.prototype && templateElement.prototype.CSPTemplate && typeof templateElement === 'function'))) {
                ...
                if (vueSlot) {
                    ...
                    console.log("template.js", templateElement, context, data, root, propName)
                    let app = aVue.createVNode({
                        render() {
                            // modify by hspark, template data v-model bind 처리가 안됨 
                                //return vueSlot[`${templateElement}`]({ data: data });
                            if ( context.ej2Instances.dataSource ) {
                                return vueSlot["" + templateElement]({  
                                    data: context.ej2Instances.dataSource[parseInt(data.index)],
                                    column: data.column,
                                });
                            } else {
                                return vueSlot[`${templateElement}`]({ data: data });
