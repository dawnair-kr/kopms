  ...
  watch: {
    options: function (nextOptions) {
      Object.keys(nextOptions).forEach(option => {
        //console.log("onBegin", option)
        if (option == "onBegin") {
          this.signaturePad.addEventListener("beginStroke", nextOptions[option], { once: true });
        } else if (option == "onEnd") {
          this.signaturePad.addEventListener("endStroke", nextOptions[option], { once: true });
        } else {
