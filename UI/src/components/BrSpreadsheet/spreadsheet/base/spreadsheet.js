...
import { mouseDown, spreadsheetDestroyed, keyUp, clearViewer, refreshSheetTabs, positionAutoFillElement, updateSortCollection, 
    isReadOnlyCells, readonlyAlert, deInitProperties } from '../common/index';
...
var Spreadsheet = /** @class */ (function (_super) {
    ...
    Spreadsheet.prototype.getRowPos = function (sheetIndex, rowIndex) {
        const sheet = getSheet(this, sheetIndex);

    };

    Spreadsheet.prototype.setRowPos = function (sheetIndex, rowIndex) {
        const sheet = getSheet(this, sheetIndex);
        if ( rowIndex == -1 ) {
            ...
            if (args) {
                ...
                if ( td.parentElement ) {
                    if ( args.result && args.result.length ) {
                        ...
                        if ( isSetZindex ) {
                            if ( nearLeftCell != null ) {
                                nearLeftCell.style.zIndex = 0;
                            } else {
                                td.style.zIndex = args.colIndex + 1;
                            }
