...
import { removeSheetTab, cMenuBeforeOpen, renameSheetTab, cut, copy, paste, focus, getUpdateUsingRaf, updateSortCollection, isReadOnlyCells, readonlyAlert } from '../common/index';
...
import { getSwapRange, getSheetIndex, moveSheet, duplicateSheet, hideShow, getRow, getColumn } from '../../workbook/index';
...
var ContextMenu = /** @class */ (function () {
    ...
    ContextMenu.prototype.selectHandler = function (args) {
        ...
        if (!selectArgs.cancel) {
            ...
            switch (args.item.id) {
                ...
                case id + '_delete_row':
                    indexes = getRangeIndexes(this.parent.getActiveSheet().selectedRange);
                    //console.log("sssssss", args.item.id.substr(id.length + 1, 6) + "Model");
    ...
    ContextMenu.prototype.beforeOpenHandler = function (args) {
        ...
        this.parent.trigger('contextMenuBeforeOpen', extend(args, {target: target}));
    ...
    ContextMenu.prototype.setNotesMenu = function (items, id) {
        if (this.parent.enableNotes) {
            ...
            if (targetElement.children.length > 0 && targetElement.children[(targetElement.children.length - 1)].className.indexOf('addNoteIndicator') > -1) {
