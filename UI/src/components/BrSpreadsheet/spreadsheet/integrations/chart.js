...
import { initiateChart, getRangeIndexes, isNumber, getSheetIndexFromAddress, checkDateFormat, refreshChartCellOnInit, workbookFormulaOperation, checkIsFormula } from '../../workbook/common/index';
...
var SpreadsheetChart = /** @class */ (function () {
    ...
    SpreadsheetChart.prototype.processChartRange = function (range, dataSheetIdx, opt) {
            ...
            if (cell) {
                ...
                if (cell.formula && isNullOrUndefined(cell.value)) {
                    _this.parent.notify(workbookFormulaOperation, {
                        action: 'refreshCalculate', value: cell.formula, rowIndex: rIdx, colIndex: cIdx, isFormula: checkIsFormula(cell.formula), sheetIndex: sheetIndex
                    });
        ...
        var seriesNameRangeSheetIndex;
        var catRangeSheetIndex;
        var valRangeSheetIndex;
        var seriesNameRangeIndexs;
        var catRangeIndexs;
        var valRangeIndexs;
        let nMinR = 1000;
        let nMaxR = -1;
        ...
        if ( opt.seriesNameRange && opt.seriesNameRange.length ) {
            seriesNameRangeSheetIndex = (opt.seriesNameRange && opt.seriesNameRange.indexOf('!') > 0) ?
                getSheetIndex(this.parent, opt.seriesNameRange.split('!')[0]) : this.parent.activeSheetIndex;

            seriesNameRangeIndexs = getRangeIndexes(opt.seriesNameRange);
            dataSheetIdx = seriesNameRangeSheetIndex;
            nMinR = Math.min(nMinR, seriesNameRangeIndexs[0]);
            nMinC = Math.min(nMinC, seriesNameRangeIndexs[1]);
            nMaxR = Math.max(nMaxR, seriesNameRangeIndexs[2]);
            nMaxC = Math.max(nMaxC, seriesNameRangeIndexs[3]);
        }

        if ( opt.catRange && opt.catRange.length ) {
            catRangeSheetIndex = (opt.catRange && opt.catRange.indexOf('!') > 0) ?
                getSheetIndex(this.parent, opt.catRange.split('!')[0]) : this.parent.activeSheetIndex;

            catRangeIndexs = getRangeIndexes(opt.catRange);
            dataSheetIdx = catRangeSheetIndex;
            nMinR = Math.min(nMinR, catRangeIndexs[0]);
            nMinC = Math.min(nMinC, catRangeIndexs[1]);
            nMaxR = Math.max(nMaxR, catRangeIndexs[2]);
            nMaxC = Math.max(nMaxC, catRangeIndexs[3]);
        }

        if ( opt.valRange && opt.valRange.length ) {
            valRangeSheetIndex = (opt.valRange && opt.valRange.indexOf('!') > 0) ?
                getSheetIndex(this.parent, opt.valRange.split('!')[0]) : this.parent.activeSheetIndex;

            valRangeIndexs = getRangeIndexes(opt.valRange);
            dataSheetIdx = valRangeSheetIndex;
            nMinR = Math.min(nMinR, valRangeIndexs[0]);
            nMinC = Math.min(nMinC, valRangeIndexs[1]);
            nMaxR = Math.max(nMaxR, valRangeIndexs[2]);
            nMaxC = Math.max(nMaxC, valRangeIndexs[3]);
        }
        ...
        isSingleRow = minr === maxr;
        isSingleCol = minc === maxc;
        ...
        return { xRange: xRange, yRange: yRange, lRange: lRange, isStringSeries: isStringSeries,
            bren: {
                seriesNameRange: {
                    isExist: seriesNameRangeSheetIndex == null ? false : true,
                    sheetIndex: seriesNameRangeSheetIndex,
                    cellIndexs: seriesNameRangeIndexs,
                },
                catRange: {
                    isExist: catRangeSheetIndex == null ? false : true,
                    sheetIndex: catRangeSheetIndex,
                    cellIndexs: catRangeIndexs,
                },
                valRange: {
                    isExist: valRangeSheetIndex == null ? false : true,
                    sheetIndex: valRangeSheetIndex,
                    cellIndexs: valRangeIndexs,
                },
            }
            ...
            while (minc <= maxc) {
                ...
                if (cell.formula && isNullOrUndefined(cell.value)) {
                    this.parent.notify(workbookFormulaOperation, {
                        action: 'refreshCalculate', value: cell.formula, rowIndex: minr, colIndex: minc, isFormula: checkIsFormula(cell.formula), sheetIndex: options.sheetIdx
                    });
                    value = cell.value || (cell.value === 0 ? 0 : null);
                }
    ...
    SpreadsheetChart.prototype.processChartSeries = function (options, sheetIndex, xRange, yRange, lRange, seriesNameRange, catRange, valRange) {
        options = options || {};
        var seriesName = '';
        var dataLabel = {};
        var val;
        ...
        var catValue;
        if ( catRange.isExist ) {
            catValue = this.getRangeData({ range: catRange.cellIndexs, sheetIdx: catRange.sheetIndex, 
                    skipFormula: true, isYvalue: true });
            //console.log("catValue", catValue);
        }

        var seriesNameValue;
        if ( seriesNameRange.isExist ) {
            //seriesNameRange.cellIndexs
            // startRow, startCol, lastRow, lastCol
            if ( seriesNameRange.cellIndexs[1] == seriesNameRange.cellIndexs[3] ) {
                // one column
                seriesNameValue = this.getRangeData({ range: seriesNameRange.cellIndexs, sheetIdx: seriesNameRange.sheetIndex, 
                    skipFormula: false, isYvalue: false });
            } else {
                var tmpValue = this.getRangeData({ range: seriesNameRange.cellIndexs, sheetIdx: seriesNameRange.sheetIndex, 
                    skipFormula: false, isYvalue: false });
                var rowGap = seriesNameRange.cellIndexs[3] - seriesNameRange.cellIndexs[1] + 1; 
                seriesNameValue = [];
                for ( var ci = 0 ; ci < tmpValue.length ; ci += rowGap ) {
                    let val = tmpValue.slice(ci, ci + rowGap).map(o => o.value).join(" ");
                    seriesNameValue.push({
                        value: val
                    });
                }
            }
            //console.log("seriesNameValue", seriesNameValue, "range", seriesNameRange.cellIndexs);
            //lValue = this.toArrayData(this.getRangeData({ range: xRange, sheetIdx: sheetIndex, skipFormula: false, isYvalue: false }));
            
        }
        var valValue;
        if ( valRange.isExist ) {
            valValue = this.getRangeData({ range: valRange.cellIndexs, sheetIdx: valRange.sheetIndex, 
                    skipFormula: false, isYvalue: true });
        ...
        if (options.isSeriesInRows) {
            if ( seriesNameRange.isExist ) {
                if ( catRange.isExist ) {
                    xValue = this.toArrayData(catValue);
                } else {
                ...
                lValue = this.toArrayData(seriesNameValue);
                diff = rDiff;
            } else {
                xValue = lRange ? this.toArrayData(this.getRangeData({ range: lRange, sheetIdx: sheetIndex, skipFormula: false, isYvalue: false })) :
                this.getVirtualXValues(cDiff + 1);
    ...
    SpreadsheetChart.prototype.focusChartRange = function (xRange, yRange, lRange, seriesNameRange, catRange, valRange) {
    ...
    SpreadsheetChart.prototype.initiateChartHandler = function (argsOpt) {
        ...
        if ( !isNullOrUndefined(chartRange.dataSheetIdx) ) {
            argsOpt.dataSheetIdx = chartRange.dataSheetIdx;
        }
        ...
        var seriesNameRange = chartRange.bren.seriesNameRange;
        var catRange = chartRange.bren.catRange;
        var valRange = chartRange.bren.valRange;

        if (sheetIdx === this.parent.activeSheetIndex && isRangeSelect) {
            this.focusChartRange(xRange, yRange, lRange, seriesNameRange, catRange, valRange);
        }
        if (argsOpt.triggerEvent && !argsOpt.isRefresh) {
            eventArgs = {
                ...
                seriesNameRange: chart.seriesNameRange,
                catRange: chart.catRange,
                valRange: chart.valRange,
                markerSettings: options.markerSettings, id: chart.id, height: chart.height, width: chart.width, 
            ...
            chart.seriesNameRange = eventArgs.seriesNameRange;
            chart.catRange = eventArgs.catRange;
            chart.valRange = eventArgs.valRange;

            chart.id = eventArgs.id;
            chart.height = eventArgs.height;
            chart.width = eventArgs.width;
        }
        var chartOptions = this.processChartSeries(options, argsOpt.dataSheetIdx, xRange, yRange, lRange, seriesNameRange, catRange, valRange);
        var primaryXAxis = {
    ...
    SpreadsheetChart.prototype.switchRowColumn = function (chartCollId, chartId, chartComp, cell) {
        ...
        var chartSeries = this.initiateChartHandler({ option: this.parent.chartColl[chartCollId],
            isRefresh: true, isSwitchRowColumn: true });
        chartComp.series = chartSeries;
    };
