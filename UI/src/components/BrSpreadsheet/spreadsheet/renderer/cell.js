...
var CellRenderer = /** @class */ (function () {
    ...
    CellRenderer.prototype.render = function (args) {
        ...
        var evtArgs = { cell: args.cell, element: args.td, address: args.address, rowIndex: args.rowIdx, 
            colIndex: args.colIdx, needHeightCheck: false, row: args.row };
    ...
    CellRenderer.prototype.getTextMeasure = function(val, font, isVert) {
        ...
        if ( isVert ) {
            ...
            let lines = val.split("\n");
            console.log("getTextMeasure", "fontHeight", fontHeight, "actualHeight", actualHeight, "lines", lines.length);
    ...
    CellRenderer.prototype.setWrapByValue = function (sheet, args) {
        if (args.cell && isNullOrUndefined(args.cell.wrap) && args.cell.value && args.cell.value.toString().includes('\n')) {
            /*
            let fontWeight = args.cell.style ? (args.cell.style.fontWeight||"normal") : "normal";
            let fontSize = args.cell.style ? (args.cell.style.fontSize||"11pt") : "11pt";
            let fontFamily = args.cell.style ? (args.cell.style.fontFamily||"Calibri") : "Calibri";
            let font = `${fontWeight} ${fontSize} ${fontFamily}`;
            let txtH = this.getTextMeasure(args.cell.value, font, true);
            console.log(args.rowIdx, args.colIdx, txtH, args.cell.value);
            */
        ...
        } else if ( args.cell && args.cell.value && args.cell.value.length && 
            ( isNullOrUndefined(args.cell.rowSpan) || args.cell.rowSpan <= 1 ) ) {
            let fontWeight = args.cell.style ? (args.cell.style.fontWeight||"normal") : "normal";
            let fontSize = args.cell.style ? (args.cell.style.fontSize||"11pt") : "11pt";
            let fontFamily = args.cell.style ? (args.cell.style.fontFamily||"Calibri") : "Calibri";
            let font = `${fontWeight} ${fontSize} ${fontFamily}`;
            /*
            let txt = this.parent.createElement('div');
            txt.style.whiteSpace = 'nowrap';
            txt.style.visibility = 'hidden';
            txt.style.font = font;
            txt.innerText = args.cell.value;
            args.td.appendChild(txt);
            setTimeout(() => {
                console.log("CellRenderer.prototype.setWrapByValue", args.cell.value, sheet.columns[args.colIdx].width, txt.offsetWidth,  args);
            });
            */
            let columnW = sheet.columns[args.colIdx].width;
            if ( args.cell.colSpan > 1 ) {
                let start = args.colIdx + 1;
                let end = args.colIdx + args.cell.colSpan - 1; 
                for ( let i = start ; i <= end ; i++ ) {
                    columnW += sheet.columns[i].width;
                }
            }
            let txtW = this.getTextMeasure(args.cell.value, font, false);
            //console.log("CellRenderer.prototype.setWrapByValue", args.cell.value, columnW, txtW, font);
            if ( columnW < txtW ) {
                let tds = args.td.parentElement.children;
                let cells = sheet.rows[args.rowIdx];
                if ( tds ) {
                    let start = cells.length - 1;
                    let end = args.colIdx + args.cell.colSpan||1 - 1;
                    for ( let i = start ; i >= end ; i--) {
                        let cell = cells[i];
                        let td = tds[i];
                        if ( td && cell && cell.value && cell.value.length ) {
                            td.style.zIndex = i + 1;            
                        }
                    }
                }
                // TODO: 우측 cell 데이터 편집시에 zIndex setting해야 함.
                
                args.td.style.overflow = "visible";
                args.td.style.zIndex = args.colIdx + 1;
                //args.cell.style = args.cell.style||{};
                //args.cell.style.overflow = "visible";
            } else {
                let cells = sheet.rows[args.rowIdx];
                let tds = args.td.parentElement.children;
                let isSetZindex = false;
                if ( cells && tds ) {
                    let start = args.colIdx - 1;
                    let end = 0;
                    for ( let i = start ; i >= end ; i--) {
                        let td = tds[i];
                        if ( td && td.style.overflow == "visible" ) {
                            isSetZindex = true;
                            break;
                        }
                    }
                ...
                if ( isSetZindex ) {
                    args.td.style.zIndex = args.colIdx;
                }
            }
        }
    };
    CellRenderer.prototype.update = function (args) {
        ...
        this.parent.refreshNode(args.td, 
            { type: formatArgs.type, result: formatArgs.formattedText, curSymbol: formatArgs.curSymbol, 
                isRightAlign: formatArgs.isRightAlign, 
                value: ((formatArgs.value || formatArgs.value === 0) ? formatArgs.value : ''), 
                isRowFill: formatArgs.isRowFill, rowIndex: args.rowIdx, colIndex: args.colIdx });
        ...
        if (args.checkNextBorder === 'Row') {
            //console.log("checkNextBorder Row", args.address);
            var borderTop = this.parent.getCellStyleValue(['borderTop'], 
                [Number(this.parent.getContentTable().rows[0].getAttribute('aria-rowindex')) - 1, args.colIdx]).borderTop;
    ...
    CellRenderer.prototype.checkMerged = function (args) {
        if (args.cell && (args.cell.colSpan < 0 || args.cell.rowSpan < 0)) {
            ...
            if (colSpan < 0 || rowSpan < 0) {
                ...
                //console.log(args.address, args.cell, args);
                if (colSpan < 0 && args.cell.style && args.cell.style.borderTop) {
                    //console.log("applyBorderTop", args.address, args.cell.value, args.cell.style.borderTop);
                    this.applyStyle(args, { borderTop: args.cell.style.borderTop });
                }
                if (rowSpan < 0 && args.cell.style && args.cell.style.borderLeft) {
                    //console.log("applyBorderLeft", args.address, args.cell.style.borderLeft);
