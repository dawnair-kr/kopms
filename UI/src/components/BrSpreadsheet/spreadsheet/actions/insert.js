...
var Insert = /** @class */ (function () {
    ...
    Insert.prototype.insert = function (actionArgs) {
        ...
        this.refreshImgElement(args.model.length, this.parent.activeSheetIndex, args.modelType, args.index, args.refRowHeights);
        if (args.isAction) {
    ...
    Insert.prototype.refreshImgElement = function (count, sheetIdx, modelType, index, refRowHeights) {
        var sheet = this.parent.sheets[sheetIdx];
        var cellObj;
        var indexes = [0, 0, sheet.usedRange.rowIndex, sheet.usedRange.colIndex];
        let rowPos = 0;
        if ( refRowHeights && refRowHeights.length ) {
            let start = Math.max(refRowHeights.length - count, 0);
            for ( let i = 0 ; i < refRowHeights.length ; i++ ) {
                if ( typeof refRowHeights[i] == "number" ) {
                    rowPos += refRowHeights[i];
                }
            }
        }
            ...
            for (var j = indexes[1]; j <= indexes[3]; j++) {
                ...
                if (cellObj && cellObj.image && cellObj.image.length > 0) {
                    if ((modelType === 'Row' && i >= index) || (modelType === 'Column' && j >= index)) {
                        //console.log("rowPos", rowPos, "sheet.usedRange", sheet.usedRange, "count", count, "len", refRowHeights ? refRowHeights.length : 0);
