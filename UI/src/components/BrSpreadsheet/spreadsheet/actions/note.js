...
import { getCell, updateCell, getSheetName } from '../../workbook/index';
/**
 * `Note` module
 */
var SpreadsheetNote = /** @class */ (function () {
    ...
    SpreadsheetNote.prototype.deleteNote = function (args) {
        ...
        if (targetElement.children.length > 0 && targetElement.children[targetElement.children.length - 1].className.indexOf('e-addNoteIndicator') > -1) {
            ...
            targetElement.removeEventListener('mouseover', function () { return _this.mouseOver(cellIndexes[0], cellIndexes[1]); });
            targetElement.removeEventListener('mouseout', function () { return _this.mouseOut(); });
    ...
    SpreadsheetNote.prototype.createNoteIndicator = function (args) {
        ...
        if (args.targetElement.children.length > 0 && (args.targetElement.children[args.targetElement.childElementCount - 1].className.indexOf('e-filter-btn') > -1 ||
            ...
            if (args.targetElement.children[args.targetElement.childElementCount - 1].className.indexOf('e-filter-btn') > -1) {
                noteIndicator.style.right = (document.getElementsByClassName('e-filter-btn')[0].getBoundingClientRect().width + 2) + 'px';
            }
            else {
                noteIndicator.style.right = (document.getElementsByClassName('e-validation-list')[0].getBoundingClientRect().width + 2) + 'px';
            }
        }
        if (!isNullOrUndefined(args.targetElement) && (args.targetElement.children.length === 0) || (args.targetElement.children.length > 0 && args.targetElement.children[args.targetElement.childElementCount - 1].className.indexOf('e-addNoteIndicator') === -1)) {
            args.targetElement.addEventListener('mouseover', function () { return _this.mouseOver(args.rowIndex, args.columnIndex); });
            args.targetElement.addEventListener('mouseout', function () { return _this.mouseOut(); });
            args.targetElement.appendChild(noteIndicator);
        }
    };
    SpreadsheetNote.prototype.mouseOver = function (rowIndex, columnIndex) {
        if ((this.isNoteVisibleOnTouch && !isNullOrUndefined(document.getElementsByClassName('e-addNoteContainer')[0])) || isNullOrUndefined(document.getElementsByClassName('e-addNoteContainer')[0])) {
    ...
    SpreadsheetNote.prototype.mouseOut = function () {
    ...
    SpreadsheetNote.prototype.updateNoteContainer = function () {
        ...
        var cellIdxs = getCellIndexes(this.parent.getActiveSheet().activeCell);
