...
var WrapText = /** @class */ (function () {
    ...
    WrapText.prototype.wrapTextHandler = function (args) {
        if (args.initial || inView(this.parent, args.range, true)) {
            if (isReadOnlyCells(this.parent, args.range)) {
                return;
            }
    ...
    WrapText.prototype.updateWrapCell = function (rowIdx, colIdx, sheet, ele) {
        //console.log("updateWrapCell", ele);
        if ( ele && ele.querySelector('.e-wrap-content') ) {
            if ( ele.style.paddingLeft != null && ele.style.paddingLeft.length ) {
                ele.querySelector('.e-wrap-content').style.paddingLeft =  ele.style.paddingLeft;
            }
        } else if (ele && !ele.querySelector('.e-wrap-content')) {
            var wrapSpan = this.wrapCell.cloneNode();
            if ( ele.style.paddingLeft != null && ele.style.paddingLeft.length ) {
                wrapSpan.style.paddingLeft =  ele.style.paddingLeft;
            }
