...
import { getColumn, isLocked, isReadOnly, validationHighlight, formulaInValidation } from '../../workbook/index';
/**
...
var DataValidation = /** @class */ (function () {
    ...
    DataValidation.prototype.removeNoteIndicator = function (td, rowIndex, columnIndex) {
        ...
        if (this.parent.allowDataValidation) {
            ...
            if (validation && validation.type === 'List') {
                ...
                if (validation.inCellDropDown) {
                    ...
                    if (!isNullOrUndefined(tdEle_1.children) && tdEle_1.children.length > 0 && tdEle_1.children[tdEle_1.childElementCount - 1].className.indexOf('e-addNoteIndicator') > -1) {
                        tdEle_1.removeChild(tdEle_1.children[tdEle_1.children.length - 1]);
                    ...
                    if (isNoteAvailable) {
                        this.parent.notify(createNoteIndicator, { targetElement: tdEle_1, rowIndex: indexes[0], columnIndex: indexes[1] });
                    }
                }
            }
                    ...
                    if (type === 'List') {
                        if (value1.indexOf('=') !== -1) {
                            ...
                            for (var idx = 0; idx < this.data.length; idx++) {
                                value_1 = (this.data[idx].text).toString();
                                var formatArgs = {
                                    formattedText: value_1, value: value_1, format: 'General',
                                    cell: { value: value_1, format: 'General' }
                                };
                                this.parent.notify(getFormattedCellObject, formatArgs);
                            ...
                            for (var idx = 0; idx < values.length; idx++) {
                                value_2 = values[idx];
                                var formatArgs = {
                                    formattedText: value_2, value: value_2, format: 'General',
                                    cell: { value: value_2, format: 'General' }
                                };
                                this.parent.notify(getFormattedCellObject, formatArgs);
                                if (args.value.toString() === formatArgs.value.toString()) {
                                    isValidate = true;
