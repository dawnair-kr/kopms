...
var WorkbookFormula = /** @class */ (function () {
            ...
            dependentCellRefs.forEach(function (dependentCellRef) {
                ...
                if (!isNullOrUndefined(fInfo)) {
                    ...
                    if (sheetNameIdx > -1) {
                        ...
                        _this.updateDataContainer([rowId - 1, colId - 1], {
                            value: formulaVal, visible: false, isNamedRange: namedRange_1, sheetId: family.tokenToParentObject.has(token) ? Number(family.tokenToParentObject.get(token))
    ...
    WorkbookFormula.prototype.calculateFormula = function (args, cellRef) {
        ...
        this.calculateInstance.valueChanged(sheet.id.toString(), cellArgs, true, usedRange, args.isRefreshing, sheet.name, args.isRandomFormula);
        ...
        args.sheets.forEach(function (sheet) {
            if (sheet.ranges.some(function (range) { return range.dataSource && (!range.info || !range.info.loadedRange ||
                ...
                _this.parent.notify(updateSheetFromDataSource, { sheet: sheet, autoDetectFormat: true, loadFromStartCell: true,
                    ...
                    isFinite: _this.parent.scrollSettings &&
                        _this.parent.scrollSettings.isFinite });
            }
            else {
                loadCompleteHandler();
            }
