...
import { templateCompiler, createVSlot } from '../utility/base-util';
/**
    ...
    if ((typeof element.content === 'string' || typeof element.content === 'function') && (!element.isTemplate || isBlazor())) {
        ...
        if (diagram.isReact) {
            ...
            if (templateFn) {
                ...
                //console.log(diagram.vueInstance.$.appContext.app, "element", element, div, nodeObject)

                let vNode = createVSlot(diagram.vueInstance.$.appContext.app, 
                    diagram.vueInstance.$slots, diagram.nodeTemplate,
                    nodeObject, div);
                //console.log(vNode);
        ...
        if (compiledString) {
            // modify by hspark
            let cloneNodes = Array.from(compiledString);
            for (var i = 0; i < cloneNodes.length; i++) {
                div.appendChild(cloneNodes[parseInt(i.toString(), 10)]);
            }
        }
    }
    else {
