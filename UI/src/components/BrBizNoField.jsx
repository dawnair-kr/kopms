...
import { bizNoFormatter, setCursor } from '../util/fmt';
...
// Utilities
import { cloneVNode, computed, nextTick, ref, watch, shallowRef } from 'vue'
...
export const makeBrBizNoFieldProps = propsFactory({
  autofocus: Boolean,
...
export default genericComponent()({
  ...
  setup (props, { attrs, emit, slots }) {
    ...
    , null, 
      val => {
        return bizNoFormatter(val, null, true, true);
      },
      val => {
        return val && val.length ? val.replace(/\-/gm, "") : null;  
      }
    )
    */

    const _inputValue = ref(bizNoFormatter(model.value, null, true, true));
    ...
    const inputValue = computed({
      ...
      set: (val) => {        
        if ( val && val.length ) {
          ...
          _inputValue.value = bizNoFormatter(val, null, true, true);

        } else if (val == null || val.length == 0 ) {
          _inputValue.value = null;
          model.value = null;
        }
        //console.log("search set", _inputValue.value, model.value);
      },
    });
    ...
    function onInput (e) {
      ...
      let fmtValue = bizNoFormatter(el.value, null, true, true);
      ...
      //console.log("model", model, e, model.externalValue, model.value, model.value ? model.value.length : 0, el.selectionEnd, el.selectionStart);
      e.preventDefault();
