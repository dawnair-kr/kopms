...
export default {
  ...
  computed: {
    actionlist () {
      ...
      for (const key in acts) { // eslint-disable-line
        ...
        //console.log("actionable", action, key, action.key, this.actions);
  ...
  methods: {
    ...
    setLoadingState (value) {
      ...
      this.setLoadingToInstance(this.$root.$.refs.dialog, value)
    },
    get (param, def) {
      if (param === undefined) {
        return def
    ...
    async onActionClick (action) {
      ...
      //console.log("onActionClick", action, closable, handler);
